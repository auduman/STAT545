---
title: "cm005-notes_and_Exercises"
author: "Aisha Uduman"
...
```{r}
filter(gapminder, (country == "Canada") | (country == "Algeria" & year < 1970 & year > 1959))
...
Order `gapminder` first by population, then by year:
...
1. Order the data frame by year, then descending by life expectancy. Descending order can be sorted using "desc"
...
```{r}
gapminder %>% arrange(gapminder, year, desc(lifeExp)) %>% select(year, lifeExp, everything())
```

Rename is like select, but KEEPS everything else. Select removes them 
#### `mutate`
